...
export const QRScannerView: React.FC<QRScannerProps> = ({ onClose, onScan }) => {
  ...
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        setCameraError("تعذر الوصول للكاميرا. تأكد من السماح بالصلاحيات أو استخدم رفع الصورة.");
        setScanning(false);
      }
    };
      ...
      reader.onload = (e) => {
          ...
          img.onload = () => {
              ...
              if (code) {
                  ...
                  alert("لم يتم العثور على كود QR صالح في الصورة. حاول مرة أخرى بصورة أوضح.");
              }
          };
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col h-[100dvh]">
      ...
      <div className="flex justify-between items-center p-6 text-white bg-black/50 absolute top-0 w-full z-20 backdrop-blur-sm">
        <h2 className="text-xl font-bold">مسح كود الاستلام</h2>
      ...
      <div className="flex-1 relative flex items-center justify-center bg-gray-900 overflow-hidden">
         ...
         {/* Scanner Overlay UI */}
         {!cameraError && (
             <div className="relative w-72 h-72 border-2 border-primary/80 rounded-3xl overflow-hidden shadow-[0_0_0_200vh_rgba(0,0,0,0.8)] z-10">
         ...
         {cameraError && (
             <div className="z-10 text-white text-center p-8 bg-gray-800 rounded-2xl max-w-xs">
                 <AlertCircle size={48} className="mx-auto text-red-500 mb-4" />
                 <p className="font-bold mb-2">الكاميرا غير متاحة</p>
                 <p className="text-sm opacity-70 mb-4">{cameraError}</p>
                 <button onClick={() => fileInputRef.current?.click()} className="bg-primary px-6 py-2 rounded-xl font-bold">
         ...
         <div className="absolute bottom-40 text-white text-center w-full px-4 z-20 pointer-events-none">
             {processingImage ? (
                 <div className="flex flex-col items-center gap-2 bg-black/60 p-4 rounded-xl backdrop-blur-md inline-block">
                     <Loader2 className="animate-spin text-primary" size={32} />
                     <p className="font-bold">جاري تحليل الصورة...</p>
                 </div>
             ) : (
                 !cameraError && (
                    <div className="bg-black/40 px-4 py-2 rounded-full inline-block backdrop-blur-sm">
                        <p className="font-bold text-sm">وجه الكاميرا نحو رمز QR الخاص بالعميل</p>
                    </div>
                 )
             )}
         </div>
      ...
      <div className="bg-black p-8 flex justify-around items-center pb-safe z-20">
         ...
         <div className="text-gray-500 text-xs w-32 text-center">
             المسح تلقائي بمجرد رؤية الكود
         </div>
      </div>
