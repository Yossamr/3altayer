...
export const StoreView: React.FC = () => {
    const { currentUser, orders, zones, createOrder, users } = useApp();
    ...
    const storeOrders = orders.filter(o => o.storeId === currentUser?.id || o.pickupAddress.includes(currentUser?.name || ''));
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        createOrder({
            ...
            deliveryAddress: { id: `temp-${Date.now()}`, title: customerName, details: addressDetails, zoneId: selectedZone },
            price: zone.price
        });
        setView('list'); setItems(''); setAddressDetails(''); setCustomerName('');
    };
            ...
            <div className="space-y-6 max-w-2xl mx-auto">
                ...
                <div className="bg-surface-light dark:bg-surface-dark p-6 rounded-3xl shadow-bold border-2 border-gray-100 dark:border-gray-700">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">اسم العميل</label>
                            <input required className="w-full p-4 bg-gray-50 rounded-xl border-2 border-gray-100 focus:border-primary outline-none" value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="مثال: أحمد محمد" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">تفاصيل الطلب</label>
                            <textarea required className="w-full p-4 bg-gray-50 rounded-xl border-2 border-gray-100 focus:border-primary outline-none" value={items} onChange={e => setItems(e.target.value)} rows={3} placeholder="قائمة الأصناف..." />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">عنوان التوصيل</label>
                            <input required className="w-full p-4 bg-gray-50 rounded-xl border-2 border-gray-100 focus:border-primary outline-none" value={addressDetails} onChange={e => setAddressDetails(e.target.value)} placeholder="الشارع، رقم العمارة، الشقة" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">المنطقة</label>
                        ...
                        <Button type="submit" fullWidth size="lg" className="mt-4">إرسال للكابتن</Button>
                    </form>
                </div>
            </div>
        )
        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="-mt-6 relative z-20 px-2 max-w-xl mx-auto">
                <div className="relative shadow-lg rounded-xl">
                    <input 
                        className="w-full h-14 pr-12 pl-4 rounded-xl border-none shadow-inner bg-white/95 text-gray-800 placeholder-gray-500 focus:ring-4 focus:ring-orange-100 font-medium outline-none transition-all" 
                        placeholder="بحث في طلبات المتجر..." 
                        type="text"
                    />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-primary dark:bg-orange-800 p-5 rounded-3xl text-white relative overflow-hidden chunk-shadow group col-span-2 md:col-span-1">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <span className="text-orange-100 text-xs font-bold">المحفظة</span>
                        </div>
                ...
                <button onClick={() => setView('create')} className="bg-surface-light dark:bg-surface-dark p-5 rounded-3xl border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-primary dark:hover:border-primary group transition-all flex flex-col justify-center items-center gap-2 col-span-2 md:col-span-1">
                    ...
                    <span className="font-bold text-gray-600 dark:text-gray-300 group-hover:text-primary">طلب جديد</span>
                </button>
            </div>
            ...
            <section>
                <div className="flex justify-between items-end mb-4 px-1">
                    <h2 className="text-xl font-black text-gray-800 dark:text-white border-r-4 border-primary pr-3 leading-none">طلبات قيد التوصيل</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {activeStoreOrders.length === 0 ? (
                        <div className="col-span-full text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-3xl">
                            <Store size={40} className="mx-auto text-gray-300 mb-2" />
                            <p className="text-gray-400 font-bold">كل الطلبات مكتملة</p>
                        </div>
                    ) : (
                        activeStoreOrders.map((order, idx) => (
                            <div key={order.id} className="bg-surface-light dark:bg-surface-dark rounded-3xl p-5 chunk-shadow-lg border-2 border-gray-100 dark:border-gray-700 relative overflow-hidden group">
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                                     <div className="flex items-center gap-2">
                                         ...
                                         <div className="text-xs">
                                             <div className="font-bold text-gray-800 dark:text-white">{order.driverId ? users.find(u => u.id === order.driverId)?.name : 'جاري البحث...'}</div>
                                             <div className="text-gray-400">الكابتن</div>
                                         </div>
                                     </div>
            ...
            <section className="grid grid-cols-3 md:grid-cols-6 gap-3">
                <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl text-center flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] font-bold text-gray-400 uppercase">الكل</span>
                </div>
                 <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl text-center flex flex-col items-center justify-center">
                    <span className="text-2xl font-black text-green-500 mb-1">{storeOrders.filter(o => o.status === 'DELIVERED').length}</span>
                    <span className="text-[10px] font-bold text-gray-400 uppercase">مكتمل</span>
                </div>
                 <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-2xl text-center flex flex-col items-center justify-center">
                    <span className="text-2xl font-black text-primary mb-1">{totalRevenue}</span>
                    <span className="text-[10px] font-bold text-gray-400 uppercase">إيراد</span>
                </div>
            </section>
        </div>
    );
